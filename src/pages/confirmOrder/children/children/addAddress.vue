<template>
  <div class="address_page">
    <v-headTop head-title="添加地址" go-back='true'></v-headTop>
    <div class="page_text_container">
      ...
      <div class="section_list">
        ...
        <div class="section_right">
          <router-link to="/confirmOrder/chooseAddress/addAddress/searchAddress" tag="div" :class="[searchAddress ? 'has_address' : 'no_address','choose_address']">{{searchAddress? searchAddress.name : '小区/写字楼/学校等'}}</router-link>
    ...
    <v-alertTip v-if="showAlert" @closeTip="closeAlert" :alertText="alertText"></v-alertTip>
...
import { getAddress, getUser, postAddAddress } from '../../../../service/getData.js'
...
export default {
  ...
  methods: {
    ...
    async addAddress() {
      ...
      //console.log(res.status=1//res.success='添加地址成功');
      //保存成功后返回上一页，否则弹出提示框
      if (res.message) {
        //保存失败
        this.showAlert = true;
